# WRM API - Backend Application

---

## Environment Stack Technologies
* [docker](https://www.docker.com/)
* [docker-compose](https://docs.docker.com/compose/)
* [Make](https://makefiletutorial.com/)
* [nginx:1.27.3-alpine](https://hub.docker.com/_/nginx)
* [php:8.4.1-fpm](https://hub.docker.com/_/php)
* [mysql:9.1](https://hub.docker.com/_/mysql)
* [bitnami/redis:7.4.1](https://hub.docker.com/r/bitnami/redis)

---

## Application Stack Technologies
* [laravel/framework:^11.31](https://laravel.com/docs/11.x/releases)
* [ramsey/uuid:^4.7](https://uuid.ramsey.dev/en/4.7.0/quickstart.html)
* [tymon/jwt-auth:^2.1](https://jwt-auth.readthedocs.io/en/develop/)
* [openai-php/laravel:^0.10.2](https://github.com/openai-php/laravel)

---

## Code Architecture
* [Laravel app](https://laravel.com/docs/11.x/structure)
* [Domain Driven Design](https://github.com/GESS32/ddd-in-laravel-skeleton)

---

## Wiki
* [Deploy project to a local environment](./wiki/setup-local-environment.md)
